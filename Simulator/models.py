...
def basic_model(k, foreign_currency, pi, alpha, s_t, f, c,
    ...
    s.t. excess_loss >= loss - v@r
         excess_loss >= 0
         gamma_i * (s_t - k) >= 0
         x + sum[y_i * gamma_i] + z - w == foreign_currency
    ...
    for i in range(n_scenarios):
        ...
        m.addConstr(x
                    + quicksum(gamma[j, i] * y[j] for j in range(len(k)))
                    + z[i] - w[i]
    ...
    for i in range(n_scenarios):
        ...
        loss.append(x * f
                    ...
                    + quicksum(gamma[j, i] * k[j] * y[j] for j in range(len(k)))
                    - benchmark_cost)
    ...
    if m.Status == GRB.OPTIMAL and print_results:
        ...
        print(f"AVG Loss: {np.mean([loss_i.getValue() for loss_i in loss]) * -1} $")
